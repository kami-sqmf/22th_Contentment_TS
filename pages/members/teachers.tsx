...
import { faAngleLeft, faAngleRight, faClock, faEnvelope } from '@fortawesome/free-solid-svg-icons'
  ...
  return (
    <div className={`flex flex-row items-center max-w-max px-3 py-2 ring-1 ring-blue rounded-lg ${className}`}>
      <select onChange={(e) => changeSection(e)}>
        <option selected={false} value={2}>現任老師</option>
        <option value={1}>一下老師</option>
        <option value={0}>一上老師</option>
      </select>
    </div>
  )
}
...
const teachersPage = () => {
  ...
  return (
    <div>
      ...
      <BreadCrumb current="/members/teachers" className="mt-8" Right={<SessionSelect className="lg:invisible" pageSelect={pageSelect} setPagesSelect={setPagesSelect} />} ></BreadCrumb>
